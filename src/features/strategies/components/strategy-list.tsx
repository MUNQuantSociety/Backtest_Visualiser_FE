...
export function StrategyList() {
    ...
    if (data.length === 0) {
        return (
            <EmptyState title="No strategies yet" description="Add one with the editor above." />
        );
    }
        ...
        <Card>
            <CardHeader className="flex-row items-start justify-between space-y-0 pb-2">
                <div className="min-w-0 space-y-1">
                    ...
                    <p className="text-muted-foreground font-mono text-xs">{strategy.className}</p>
                </div>
                <Badge variant={statusVariant[strategy.status]}>{strategy.status}</Badge>
            </CardHeader>

            <CardContent className="space-y-3">
                <p className="text-muted-foreground line-clamp-2 text-sm">{strategy.description}</p>

                <div className="flex flex-wrap gap-1">
                    {strategy.tags.map((tag) => (
                        <span
                            key={tag}
                            className="bg-secondary text-muted-foreground rounded px-1.5 py-0.5 text-xs"
                        >
                            {tag}
                        </span>
                    ))}
                ...
                <dl className="grid grid-cols-3 gap-2 border-t pt-3 text-sm">
                    ...
                    <div>
                        <dt className="text-muted-foreground text-xs">Best Sharpe</dt>
                        <dd className="font-medium">
                            {strategy.bestSharpe === null ? '—' : formatNumber(strategy.bestSharpe)}
                        </dd>
                    </div>
                    <div>
                        <dt className="text-muted-foreground text-xs">Best return</dt>
                        <dd
                            className={
            ...
            to an empty list. */}
                {hasRuns ? (
                    ...
                    <p className="text-muted-foreground text-sm">Never tested</p>
                )}
            </CardContent>
        </Card>
    );
