...
import { groupByOrigin, strategyColorIndex, type StrategyFilter } from '../strategy-filter';
...
export function StrategyPicker({
  ...
  function renderStrategy(strategy: Strategy) {
      ...
      return (
        <div key={strategy.id} className="relative">
          <button
            ...
            className={cn(
              'flex w-full max-w-full min-w-0 flex-wrap items-center justify-between overflow-hidden rounded-md border px-3.5 py-2 pr-9 text-left text-[13px] [overflow-wrap:anywhere] text-muted-foreground transition-colors',
              selected
      ...
      <div key={strategy.id} className="relative">
        <button
          ...
          className={cn(
            ...
            selected ? 'border-primary shadow-[inset_3px_0_0_var(--primary)]' : 'hover:bg-muted/60',
          )}
        >
          <div className="flex items-center gap-2">
            <span
              ...
              style={{
                background:
                  colorIndex === null ? 'var(--border-strong)' : seriesColor(palette, colorIndex),
              }}
              aria-hidden
            />
            <span
          ...
          {!draft ? (
            <dl className="tabular mt-2.5 grid grid-cols-3 gap-2 text-xs">
              ...
              <div>
                ...
                <dd>{strategy.bestSharpe === null ? '—' : formatNumber(strategy.bestSharpe)}</dd>
              </div>
              <div>
                <dt className="text-[9px] font-medium tracking-[0.06em] text-muted-foreground uppercase">
                  Last run
    ...
    <div className="space-y-3">
      ...
      {isLoading ? (
        ...
        <div
          ...
          // The Library's viewport-aware scroll region, shared with its runs
          // list; focusable so a keyboard user can scroll it.
          className="library-scroll-region strategy-picker-scroll space-y-4"
          tabIndex={0}
        >
          {groupByOrigin(shown).map((group) => (
            <div key={group.origin} role="group" aria-label={group.label} className="space-y-1.5">
              <p
                aria-hidden
                className="flex items-center justify-between text-[10px] font-medium tracking-[0.06em] text-muted-foreground uppercase"
              >
                {group.label}
                <span className="tabular">{String(group.items.length)}</span>
              </p>
              {group.items.map(renderStrategy)}
            </div>
          ))}
          {shown.length === 0 ? (
