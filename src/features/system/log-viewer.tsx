...
export function LogViewer() {
    ...
    return (
        <div className="space-y-3">
            ...
            {isError ? (
                <p className="text-muted-foreground py-8 text-center text-sm">{error.message}</p>
            ) : null}
            ...
            {entries.length > 0 ? (
                <div
                    className="bg-muted/30 max-h-128 overflow-auto rounded-md border"
                    role="log"
                    aria-live="polite"
                        ...
                        <tbody>
                            {entries.map((entry) => (
                                <tr key={entry.id} className="border-b last:border-0">
                                    <td className="tabular text-muted-foreground w-20 px-3 py-1.5 align-top">
                                        {timeFormat.format(new Date(entry.timestamp))}
                                    </td>
                                    <td
                                        className={cn(
                                            'w-20 px-2 py-1.5 align-top',
                                            levelClass[entry.level],
                                        )}
                                    >
                                        {entry.level}
                                    </td>
                                    <td className="text-muted-foreground w-44 truncate px-2 py-1.5 align-top">
                                        {entry.logger}
                                    </td>
                                    <td
                                        className={cn(
            ...
            {data?.truncated ? (
                <p className="text-muted-foreground text-xs">
                    Showing the most recent {data.entries.length} lines. Older entries are on the
                    host.
                </p>
            ) : null}
        </div>
