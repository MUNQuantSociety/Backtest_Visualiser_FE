...
/**
 * Demo data for the Backtest Visualiser, served when `VITE_USE_FIXTURES=true`.
 *
 * Same stopgap caveat as the portfolios fixtures: the real answer is a Prism
 * mock generated from the OpenAPI spec. Until that exists this keeps `/` — the
 * app's landing route — rendering instead of erroring, which matters because
 * the demo is the first thing anyone sees.
 *
...
function summaryFrom(detail: BacktestDetail): BacktestSummary {
  const { metrics: _metrics, equityCurve: _curve, trades: _trades, parameters: _params, ...summary } = detail;
  return summary;
}

export function fixtureBacktests(): BacktestSummary[] {
  return BLUEPRINTS.map((blueprint) => summaryFrom(detailFor(blueprint)));
}

export function fixtureBacktest(id: string): BacktestDetail {
  const blueprint = BLUEPRINTS.find((candidate) => candidate.id === id);
  if (!blueprint) {
    throw new Error(`No fixture backtest with id "${id}"`);
  }
  return detailFor(blueprint);
}
