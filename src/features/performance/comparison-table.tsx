        ...
        <thead>
          <tr className="border-b text-xs text-muted-foreground">
            ...
            {backtests.map((backtest, index) => (
              <th key={backtest.id} scope="col" className="py-2 pr-4 text-right font-medium">
                <span className="inline-flex items-center gap-1.5">
                  ...
                  />
                  <span className="max-w-[10rem] truncate text-foreground">{backtest.name}</span>
                </span>
              </th>
            ))}
          </tr>
            ...
            return (
              <tr key={row.label} className="border-b border-border/40 last:border-b-0">
                ...
                {values.map((value, index) => {
                  const isBest = best !== null && value === best && backtests.length > 1;
                  return (
                    <td
                      key={backtests[index]?.id ?? index}
                      className={cn(
                        'py-2 pr-4 text-right font-mono tabular-nums',
                        isBest ? 'font-semibold text-[var(--profit)]' : 'text-muted-foreground',
                      )}
                    >
                      {row.format(value)}
