...
export function MetricsTable({ detail, isLoading = false }: MetricsTableProps) {
  ...
  if (!detail) {
    return <p className="py-8 text-center text-sm text-muted-foreground">No metrics available.</p>;
  }
        ...
        {sections.map((section) => (
          ...
          <tbody key={section.category} className="border-b last:border-b-0">
            {section.rows.map((row, index) => {
              const tone =
                row.signed && typeof row.value === 'number' && Number.isFinite(row.value)
              ...
              return (
                <tr key={row.label} className="border-b border-border/40 last:border-b-0">
                  ...
                  <td className={cn('py-2 text-right font-mono tabular-nums', toneClass[tone])}>
                    {formatValue(row)}
                  </td>
                </tr>
              );
