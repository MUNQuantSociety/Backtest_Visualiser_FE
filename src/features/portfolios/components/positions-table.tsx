...
export function PositionsTable({ positions, isLoading = false }: PositionsTableProps) {
  ...
  if (!positions || positions.length === 0) {
    return <p className="py-8 text-center text-sm text-muted-foreground">No open positions.</p>;
  }
          ...
          {positions.map((position) => {
            ...
            return (
              <tr key={position.ticker} className="border-b last:border-0">
                ...
                <td className={cn('tabular py-2 pr-4 text-right font-medium', toneClass[tone])}>
